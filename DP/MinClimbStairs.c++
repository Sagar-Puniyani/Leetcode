...
int solveDP( vector<int> &cost ,vector<int> &dp ,  int n ){
    ...
    dp[n] = (min(solveDP(cost , dp , n-1) , solveDP(cost , dp , n-2))) + cost[n];
    return  dp[n];
}
...
int minCostClimbingStairsDP(vector<int>& cost) {
    ...
    int res = min ( solveDP(cost , dp , n-1 )  ,
                    solveDP(cost , dp ,n-2 ) );
    return res;
}
...
int minCostClimbingStairsDPTab(vector<int>& cost) {

    int n = cost.size();
    cout << " N = " << n << endl;
    
    int res = min ( solveDPTab(cost , n-1 )  ,
                    solveDPTab(cost ,n-2 ) );
    return res;
}
...
int minCostClimbingStairsDPTabSP_OPT(vector<int>& cost) {
